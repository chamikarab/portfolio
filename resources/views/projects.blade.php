    ...
    <h2 class="text-3xl font-bold mt-10 mb-20 text-center">
            ...
            @forelse ($projects as $project)
                <div class="p-6 rounded-lg shadow-lg transition duration-300 ease-in-out project-card-2">
                    ...
                    <img src="{{ asset('storage/' . $project->image) }}" alt="{{ $project->name }}" 
                         class="w-full h-48 object-cover rounded-lg mb-4"> <!-- Ensures image fits in a 48px height box -->
        ...
        @if ($projects->isEmpty())
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 mt-10">
                @for ($i = 1; $i <= 3; $i++)
                    <div class="p-6 rounded-lg shadow-lg transition duration-300 ease-in-out project-card-2">
                        <div class="w-full h-48 bg-gray-700 rounded-lg mb-4"></div> <!-- Placeholder -->
